@extends('layouts.dashboard.admin', ['title' => 'Freezolar Refrigeração - Administração - Catálogo - Editar Produto'])

@section('content')
<div class="bg-body-light">
    <div class="content content-full p-1">
        <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
            <h1 class="flex-sm-fill h3 my-2 pl-3">Editar Produto</h1>
            <nav class="flex-sm-00-auto ml-sm-3 pr-3" aria-label="breadcrumb">
                <ol class="breadcrumb breadcrumb-alt">
                    ...
                    <li class="breadcrumb-item" aria-current="page">
                        <a class="link-fx" href="{{ route('dashboard.admin.site.catalogo.produtos.edit', $produto->id) }}">Editar</a>
                    </li>
                </ol>
            </nav>
        </div>
        ...
        <div class="block-content block-content-full">
            <div class="row">
                <div class="col-12 col-md-12 col-lg-12">
                    <form class="mb-5" method="POST" action="{{ route('dashboard.admin.site.catalogo.produtos.update', $produto->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="row">
                            <div class="col-lg-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="codigo" name="product[codigo]" placeholder="Código do Produto..." value="{{$produto->codigo}}">
                                </div>
                            </div>
                            <div class="col-lg-8">
                                <div class="form-group">
                                    <label for="nome">Nome do Produto: <small style="color:red;"> * </small></label>
                                    <input type="text" class="form-control" id="nome" name="product[nome]" placeholder="Nome do Produto..." required value="{{$produto->nome}}">
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-lg-3">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="modelo" name="product[modelo]" placeholder="Modelo do Produto..." value="{{ $produto->modelo}}">
                                </div>
                            </div>
                            <div class="col-lg-3">
                                <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <div class="input-group">
                                            ...
                                            <input type="text" class="form-control money" id="preco" name="product[preco]" value="{{ $produto->preco }} ">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-3">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="exibir" name="product[exibir]" style="width: 100%;" >
                                        <option value="1" {{$produto->exibir == 1 ? 'selected' : ''}}>Sim</option>
                                        <option value="0" {{$produto->exibir == 0 ? 'selected' : ''}}>Não</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-lg-3">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="mostrar_preco" name="product[mostrar_preco]" style="width: 100%;">
                                        <option value="0" {{$produto->mostrar_preco == 0 ? 'selected' : ''}}>Não</option>
                                        <option value="1" {{$produto->mostrar_preco == 1 ? 'selected' : ''}}>Sim</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="js-select2 form-control" id="category_id" name="category_id[]" style="width: 100%;" data-placeholder="Selecione mais de uma se for preciso" title="Selecione mais de uma se for preciso" multiple>
                                        ...
                                        @foreach($categorias as $categoria)
                                            <option value="{{$categoria->id}}" {{ $produto->categorias->contains($categoria) ? 'selected' : ''}}>{{$categoria->category}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-lg-12">
                                <div class="row">
                                    ...
                                    <div class="col-md-2 text-center">
                                        <img class='m-2 border' width='120' height='120' src='/storage/{{$foto->path}}'>
                                        <a  href="{{ route('dashboard.admin.site.catalogo.produtos.fotos.destroy', ['produto' => $produto->id, 'foto' => $foto->id] ) }}" type="submit" class="btn btn-sm btn-danger">Remover</a>
                                    </div>
                                    @endforeach
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="form-group">
                                    ...
                                    <textarea id="ckeditor" name="product[descricao]">{!! $produto->descricao !!}</textarea>
                                </div>
                            </div>
                        </div>
                        <hr>
                            ...
                            <div class="col-lg-12">
                                <div class="form-group">
                                    <table id="item-add" style="width:100%;">
                                        @if(!is_null($produto->atributos) && $produto->atributos->count() > 0)
                                        @for($i = 0; $i < $produto->atributos->count(); $i++)
                                        <tr class="list-item">
                                            <td>
                                                <div class="row">
                                                    <input type="hidden" class="form-control" maxlength="255" name="atributos[{{$i}}][atributo_id]" placeholder="Ex.: Medida, Peso, Voltagem" value="{{$produto->atributos[$i]->id}}">
                                                    <div class="col-12 col-md-5 col-lg-4">
                                                        <label for="curso" class="col-form-label p-0">Atributo</label>
                                                        <input type="text" class="form-control" maxlength="255" name="atributos[{{$i}}][atributo]" placeholder="Ex.: Medida, Peso, Voltagem" value="{{$produto->atributos[$i]->atributo}}">
                                                    </div>
                                                    <div class="col-12 col-md-5 col-lg-7">
                                                        <label for="curso" class="col-form-label p-0">Valor</label>
                                                        <input type="text" class="form-control" maxlength="255" name="atributos[{{$i}}][valor]" placeholder="Ex.: 1.85M, 15KG, 127V" value="{{$produto->atributos[$i]->valor}}">
                                                    </div>
                                        ...
                                        <tr class="list-item">
                                            <td>
                                                <div class="row">
                                                    ...
                                                    <div class="col-12 col-md-5 col-lg-7">
                                                        <label for="curso" class="col-form-label p-0">Valor</label>
                                                        <input type="text" class="form-control" maxlength="255" name="atributos[0][valor]" placeholder="Ex.: 1.85M, 15KG, 127V" value="">
                                                    </div>
                                                    <div class="col-12 col-md-2 col-lg-1">
                                                        <label class="col-form-label p-0"></label>
                                                        <div class="form-group">
