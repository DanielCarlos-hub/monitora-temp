@extends('layouts.dashboard.monitoramento', ["title" => "Freezolar Refrigeração - Monitoramento - Clientes"])
            ...
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center mb-2 text-center text-sm-left">
                ...
                <div class="flex-sm-00-auto mt-3 mt-sm-0 ml-sm-3">
                    <span class="d-inline-block js-appear-enabled animated fadeIn" data-toggle="appear" data-timeout="350">
                        <a class="btn btn-success px-4 py-2 js-click-ripple-enabled" href="{{ route('dashboard.monitoramento.clientes.equipamentos.create', $tenant->id) }}">
    ...
    <div class="content content-boxed">
        <div class="row items-push text-center">
            <div class="col-6 col-md-3">
                ...
                <a class="link-fx font-size-h3" href="{{ route('dashboard.monitoramento.clientes.equipamentos.index', $tenant->id)}}">{{$countEquipamentos}}</a>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="col-12 col-md-6 col-lg-4">
            <div class="block equipamento" data-equip-id="{{$equipamento->id}}">
                <div class="block-header block-header-default">
                    <h3 class="block-title"><a href="{{route('dashboard.monitoramento.clientes.equipamentos.show', ['cliente' => $tenant->id, 'equipamento' => $equipamento->id]) }}">{{$equipamento->nome_equipamento}} <small>{{$equipamento->num_placa}}</small></a></h3>
                    <a href="{{ route('dashboard.monitoramento.clientes.equipamentos.edit', ['cliente' => $tenant->id, 'equipamento' => $equipamento->id]) }}" title="Editar" role="button" class="btn btn-sm mr-1"><i class="fas fa-edit"></i></a>
                    <a href="{{ route('dashboard.monitoramento.clientes.equipamentos.notificacoes', ['cliente' => $tenant->id, 'equipamento' => $equipamento->id]) }}" title="Notificações" role="button" class="btn btn-sm mr-1"><i class="si si-screen-smartphone"></i></a>
                    <a href="{{ route('dashboard.monitoramento.clientes.equipamentos.logs', ['cliente' => $tenant->id, 'equipamento' => $equipamento->id]) }}" title="Logs" role="button" class="btn btn-sm mr-1"><i class="fas fa-exclamation-triangle"></i></a>
...
@section('javascript')
    ...
    $.each(dataList, function(index, value){
        ...
        (function update() {
            ...
            $.ajax({
                url: '/monitoramento/cliente/'+'{{$tenant->id}}'+'/equipamento/'+value+'/temperatura',
                dataType: 'json',
    ...
    function clearTemperaturas(id)
    {
        ...
        var getEquipamento = "{{ route('dashboard.monitoramento.equipamento.json', ['cliente' => $tenant->id, 'equipamento' => ':id']) }}";
        ...
        $.getJSON(getEquipamento, function(data) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    var clearUrl = "{{ route('dashboard.monitoramento.clientes.equipamentos.clear', ['cliente' => $tenant->id, 'equipamento' => ':id']) }}"
